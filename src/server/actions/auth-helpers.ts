 ...
 * - React cache()로 동일 요청 내 중복 supabase.auth.getUser() 호출 방지
 */
...
/**
 * 요청 단위 캐싱된 현재 사용자 조회 (내부용)
 * React cache()로 감싸서 동일 렌더링 패스 내 1회만 실행
 */
const _getCachedCurrentUser = cache(async (): Promise<AuthUser | null> => {
  if (isDevMode()) {
    return DEV_USER;
  }
...
export async function requireAuth(): Promise<AuthUser> {
  const user = await _getCachedCurrentUser();
  if (!user) {
