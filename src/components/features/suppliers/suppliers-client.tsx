...
export function SuppliersClient({ initialSuppliers }: SuppliersClientProps) {
  ...
  const fetchSuppliers = useCallback(async (searchTerm?: string) => {
    try {
      setIsLoading(true);
      const result = await getSuppliers({ search: searchTerm || undefined });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button size="sm" variant="outline" onClick={handleExportExcel} disabled={excelLoading || suppliers.length === 0}>
            {excelLoading ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Download className="mr-1.5 h-4 w-4" />}
            엑셀 다운로드
          </Button>
          <Button size="sm" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={excelLoading}>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <TabsContent value="card">
              <SupplierCardView suppliers={suppliers} onEdit={handleEdit} onDelete={handleDelete} />
            </TabsContent>
          </>
        )}
      </Tabs>
